            ...
            <?php if (isset($corp_data['corporation']['法人沿革']) and is_array($corp_data['corporation']['法人沿革'])) { ?>
              <div id="deau_api-shortcode-history">
                ...
                <ul class="deau_api-shortcode-history-codes">
                  <li><textarea name="deau_api_shortcode_history[header]" id="deau_api-shortcode-history-header" cols="100" rows="1" placeholder="<dl>"><?php if ($deau_option_shortcode_history) {
                                                                                                                                                          echo wp_kses_post($deau_option_shortcode_history['header']);
                                                                                                                                                        } ?></textarea></li>
                  <li><textarea name="deau_api_shortcode_history[body]" id="deau_api-shortcode-history-body" cols="100" rows="10" placeholder="<dt>['year']年（['year_jp']）['month']月</dt><dd>['event']</dd>"><?php if ($deau_option_shortcode_history) {
                                                                                                                                                                                                              echo wp_kses_post($deau_option_shortcode_history['body']);
                                                                                                                                                                                                            } ?></textarea></li>
                  <li><textarea name="deau_api_shortcode_history[footer]" id="deau_api-shortcode-history-footer" cols="100" rows="1" placeholder="</dl>"><?php if ($deau_option_shortcode_history) {
                                                                                                                                                            echo wp_kses_post($deau_option_shortcode_history['footer']);
                                                                                                                                                          } ?></textarea></li>
      ...
      if ($slug_check) {
        while ($slug_check) {
          if (mb_strpos($code, "['") !== false) {
            ...
            $corp_data_word = isset($corp_data['corporation']['基本情報'][$word]) ? $corp_data['corporation']['基本情報'][$word] : null;
            if ($corp_data_word) {
